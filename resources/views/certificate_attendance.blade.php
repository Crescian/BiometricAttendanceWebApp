        ...
        <div id="addCertificateModal"
            ...
            <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl p-6">
                ...
                <div class="flex justify-between items-center border-b pb-2 mb-4">
                    <h3 class="text-lg font-semibold text-gray-800">Add New Attendance Certificate</h3>
                    <button onclick="closeAddCertificateModal()"
               ...
               focus:ring-blue-500 focus:border-blue-500 text-sm"
                    ...
                    <div class="flex items-center space-x-2 mt-3">
                        <input id="is_cutoff" name="is_cutoff" type="checkbox"
                            class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded">
                        <label for="is_cutoff" class="text-sm font-semibold text-gray-700">Is Cutoff</label>
                    </div>
                </div>
                <!-- Attachment Upload -->
                {{-- <div class="mt-6">
    ...
    <script>
        ...
        function approveCertificate(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/certificateOfAttendance/${id}/approved`,
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function cancelCertificate(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/certificateOfAttendance/${id}/cancelled`,
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function loadEmployeeName() {
            ...
            $('#employee_name').append('<option value="all" class="text-red-500 font-semibold">All Employees</option>');
            $.ajax({
                url: "{{ route('department.getUserDepartment') }}",
                type: 'GET',
                dataType: 'json',
        ...
        function loadCertificateOfAttendanceCounts() {
            $.ajax({
                url: "{{ route('department.getUserDepartment') }}",
                type: 'GET',
                dataType: 'json',
        ...
        function openAddCertificateModal() {
            $('#is_cutoff').prop('checked', true);
            document.getElementById('addCertificateModal').classList.replace('hidden', 'flex');
        }
        ...
        function showSkippedModal(skippedList) {
            ...
            if (skippedList.length === 0) {
                ...
                skippedList.forEach(item => {
                    ...
                    div.classList.add("flex", "justify-between", "items-center", "bg-white", "border", "rounded-md",
                        "p-2", "shadow-sm");
                    div.innerHTML = `
            ...
            $.each(formattedD, function(index, date) {
                ...
                if (inTime || outTime) {
                    attendanceArray.push({
                        ...
                        date: date,
                        earliest_time: inTime + ':00',
                        latest_time: outTime + ':00',
                        others: $('#others').val(),
                        reason: $('#reasons').val(),
                        weekday: weekday,
                        is_cutoff: $('#is_cutoff').is(':checked') // ✅ returns true or false
                    });
                }
            });
            if (attendanceArray.length > 0) {
                $.ajax({
                    url: "{{ route('certificateOfAttendance.store') }}",
                    method: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        // });
                        // 🟢 Show modal if there are skipped records
